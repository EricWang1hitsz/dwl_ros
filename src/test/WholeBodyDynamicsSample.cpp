...
//	std::string yarf_file = DWL_SOURCE_DIR"/config/hyq.yarf";
    ...
    //std:: cout << ws.getJointPosition() << std::endl;


    // Computing the ID
    dwl::rbd::BodyVector6d grf; // ground reaction force.
...
//	std::cout << std::endl;
    ...
    Eigen::MatrixXd inertial_mat = wdyn.computeJointSpaceInertiaMatrix(ws.base_pos, ws.joint_pos);
    std::cout << "--------------- Joint Space Inertia Matrix --------------" << std::endl;
    std::cout << "Joint space inertia matrix size: " << inertial_mat.rows() << " x " << inertial_mat.cols() << std::endl;
    std::cout << inertial_mat << " = inertial matrix" << std::endl;

    // Computing the centroidal inertia matrix
    dwl::rbd::Matrix6d com_inertial_mat = wdyn.computeCentroidalInertiaMatrix(ws.base_pos, ws.joint_pos);
    std::cout << "--------------- _Centroidal Inertia Matrix --------------" << std::endl;
    std::cout << "centroidal inertia matrix size: " << com_inertial_mat.rows() << " x " << com_inertial_mat.cols() << std::endl;
    std::cout << com_inertial_mat << " = centroidal inertial matrix" << std::endl;

    // Compute nonlinear effects force
    Eigen::VectorXd nonlinear_effects = wdyn.computeNonlinearEffectsForce(ws.base_pos, ws.joint_pos, ws.base_vel, ws.joint_vel);
    std::cout << "--------------- nonlinear effects force --------------" << std::endl;
    std::cout << "nonlinear effects vector size: " << nonlinear_effects.size() << " x 1 " <<std::endl;
    std::cout << nonlinear_effects << std::endl;
